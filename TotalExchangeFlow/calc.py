# AUTOGENERATED! DO NOT EDIT! File to edit: 03_calc.ipynb (unless otherwise specified).

__all__ = ['logger', 'calc_volume_transport', 'calc_salt_transport', 'convert_q_to_Q', 'sort_1dim', 'sort_2dim',
           'calc_bulk_values']
...
def convert_q_to_Q(self, var_q, q, var_q2 = None):
    ...
    if len(q.shape) == 1:
        ...
        out_Q_tmp = np.cumsum(np.cumsum(q[:,::-1,::-1],axis=1),axis=2)[:,::-1,::-1]*delta_var2*delta_var
        out_Q[:,:-1,:-1] = out_Q_tmp
        var_Q = np.append(var_q-0.5*delta_var,var_q[-1]+0.5*delta_var)
        var_Q2 = np.append(var_q2-0.5*delta_var2,var_q2[-1]+0.5*delta_var2)
